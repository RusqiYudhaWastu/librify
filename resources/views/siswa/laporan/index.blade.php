...
<body class="antialiased flex h-screen w-full overflow-hidden text-left font-jakarta" 
            ...
            <div class="mx-auto w-full max-w-[1550px] space-y-8">
                ...
                <div class="flex flex-col xl:flex-row xl:items-end justify-between gap-6">
                    <div class="text-left w-full xl:w-auto">
                        <div class="flex items-center gap-3 mb-4">
                            <h2 class="text-3xl font-black text-gray-900 tracking-tight uppercase leading-none">Riwayat & Laporan Kelas</h2>
                        ...
                        <div class="flex bg-gray-200/50 p-1.5 rounded-2xl gap-1 w-fit border border-gray-100">
                            <button @click="activeTab = 'riwayat'; searchQuery = ''" :class="activeTab === 'riwayat' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-blue-500'" class="px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2">
                                <i class="bi bi-clock-history text-sm"></i> Riwayat Kelas
                            </button>
                ...
                <div x-show="activeTab === 'riwayat'" x-transition>
                    ...
                    <div class="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm mb-6">
                        <form action="{{ route('siswa.laporan.export') }}" method="GET" class="flex flex-col md:flex-row items-end gap-4">
                            <div class="flex-1 w-full text-left">
                                <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 block ml-2">Dari Tanggal</label>
                                <input type="date" name="start_date" class="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl font-bold text-xs outline-none focus:ring-4 focus:ring-blue-500/10">
                            </div>
                            <div class="flex-1 w-full text-left">
                                <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 block ml-2">Sampai Tanggal</label>
                                <input type="date" name="end_date" class="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl font-bold text-xs outline-none focus:ring-4 focus:ring-blue-500/10">
                            </div>
                            <button type="submit" class="w-full md:w-auto px-8 py-3.5 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-600 transition-all shadow-lg flex items-center justify-center gap-2">
                    ...
                    <div class="rounded-[2.5rem] bg-white shadow-sm border border-gray-100 overflow-hidden text-left">
                        <div class="p-8 border-b border-gray-50 flex items-center justify-between bg-gray-50/30">
                            <h3 class="text-[10px] font-black text-gray-800 uppercase tracking-[0.2em]">Log Peminjaman Kelas</h3>
                        </div>
                                    ...
                                    <tr class="group hover:bg-gray-50/50 transition-all duration-200"
                                        ...
                                        <td class="px-8 py-5">
                                            <div class="flex items-center gap-4">
                                                <div class="w-11 h-11 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center font-black text-xs shadow-inner uppercase">
    ...
    <div x-show="modalTambah" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div x-show="modalTambah" x-transition.scale.95 class="relative w-full max-w-lg bg-white rounded-[3rem] shadow-2xl p-10 border border-white text-left overflow-y-auto max-h-[90vh] custom-scroll">
            ...
            <form action="{{ route('siswa.laporan.problem') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
    ...
    <div x-show="modalDetail" x-cloak class="fixed inset-0 z-[120] flex items-center justify-center p-4">
        ...
        <div x-show="modalDetail" x-transition.scale.95 class="relative w-full max-w-lg bg-white rounded-[3rem] shadow-2xl p-10 border border-white flex flex-col max-h-[90vh] text-left leading-none overflow-y-auto custom-scroll">
            ...
            <div class="flex justify-between items-start mb-8 text-left">
                <div class="text-left">
                    <p class="text-[10px] font-black uppercase tracking-[0.2em] mb-3" 
                       :class="detailData.type === 'loan' ? 'text-blue-500' : 'text-red-500'" 
                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div class="p-6 bg-slate-50 rounded-[2rem] border border-slate-100">
                                ...
                                <p class="text-xs font-black text-blue-600 uppercase" x-text="detailData.status"></p>
                            </div>
                        </div>
                        <div class="p-6 bg-blue-50/50 rounded-[2rem] border border-blue-100 space-y-3">
                            <div class="flex justify-between border-b border-blue-100 pb-2">
                                <span class="text-[10px] font-bold text-blue-600 uppercase">Dipinjam</span>
                                <span class="text-xs font-black text-blue-900" x-text="detailData.date"></span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-[10px] font-bold text-blue-600 uppercase">Dikembalikan</span>
                                <span class="text-xs font-black text-blue-900" x-text="detailData.return_date"></span>
                            </div>
                        </div>
            ...
            <button @click="modalDetail = false" class="w-full mt-8 py-5 bg-slate-900 text-white rounded-[2rem] font-black text-[10px] uppercase tracking-[0.2em] shadow-xl hover:bg-blue-600 active:scale-95 transition-all">Tutup Informasi</button>
        </div>
    </div>
