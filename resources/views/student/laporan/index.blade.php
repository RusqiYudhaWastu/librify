...
<head>
    ...
    <title>Riwayat & Laporan - Librify Member</title>
    ...
    <style>
        ...
        .custom-scroll::-webkit-scrollbar-thumb { background: #0891b2; border-radius: 20px; }
        ...
        /* Styling khusus untuk custom Kalender Input (Tema Cyan) */
...
<body class="antialiased flex h-screen w-full overflow-hidden text-left font-jakarta" 
            ...
            <div class="mx-auto w-full max-w-[1550px] space-y-6 lg:space-y-8">
                ...
                <div class="flex flex-col xl:flex-row xl:items-end justify-between gap-5 lg:gap-6">
                    <div class="text-left w-full xl:w-auto">
                        <div class="flex items-center gap-3 mb-4">
                            <h2 class="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight uppercase leading-none">Riwayat & Laporan</h2>
                            <span class="hidden sm:inline-block px-3 py-1.5 bg-cyan-600 text-white text-[9px] sm:text-[10px] font-black rounded-lg uppercase tracking-widest shadow-md shadow-cyan-100">
                                Akun Member
                            </span>
                        </div>
                        ...
                        <div class="w-full overflow-x-auto hide-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0">
                            <div class="flex items-center bg-gray-200/50 p-1.5 rounded-xl sm:rounded-2xl gap-1 w-max sm:w-fit border border-gray-100">
                                <button @click="activeTab = 'riwayat'; searchQuery = ''" :class="activeTab === 'riwayat' ? 'bg-white text-cyan-600 shadow-sm' : 'text-slate-500 hover:text-cyan-500'" class="px-5 sm:px-8 py-2.5 sm:py-3 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1 sm:gap-2 whitespace-nowrap">
                                    <i class="bi bi-clock-history text-xs sm:text-sm"></i> Riwayat Sirkulasi
                                </button>
                                <button @click="activeTab = 'laporan'; searchQuery = ''" :class="activeTab === 'laporan' ? 'bg-white text-red-600 shadow-sm' : 'text-slate-500 hover:text-red-500'" class="px-5 sm:px-8 py-2.5 sm:py-3 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1 sm:gap-2 whitespace-nowrap">
                                    <i class="bi bi-exclamation-triangle-fill text-xs sm:text-sm"></i> Lapor Masalah
                                </button>
                                
                                <div class="hidden sm:block h-5 w-px bg-gray-300 mx-1 sm:mx-2"></div>
                                
                                <button @click="activeTab = 'rapot'; searchQuery = ''; setTimeout(initChart, 100)" :class="activeTab === 'rapot' ? 'bg-gradient-to-r from-cyan-600 to-blue-600 text-white shadow-sm shadow-blue-200 border-blue-400/50' : 'text-slate-500 hover:text-cyan-600 border-transparent'" class="px-5 sm:px-8 py-2.5 sm:py-3 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1 sm:gap-2 border border-transparent whitespace-nowrap">
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 w-full xl:w-auto mt-2 xl:mt-0">
                        <div x-show="activeTab !== 'rapot'" class="relative w-full sm:w-64 md:w-80 group">
                            <i class="bi bi-search absolute left-4 sm:left-5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-cyan-600 transition-colors"></i>
                            <input type="text" x-model="searchQuery" placeholder="Cari data..." 
                                   class="w-full pl-10 sm:pl-12 pr-4 sm:pr-6 py-3 sm:py-3.5 bg-white border border-gray-100 rounded-xl sm:rounded-[1.5rem] outline-none font-bold text-[11px] sm:text-xs shadow-sm focus:ring-2 sm:focus:ring-4 focus:ring-cyan-500/10 transition-all placeholder:text-gray-300">
                        </div>
                ...
                <div x-show="activeTab === 'riwayat'" x-transition>
                    ...
                    <div class="bg-white p-5 sm:p-6 rounded-2xl sm:rounded-[2rem] border border-gray-100 shadow-sm mb-4 sm:mb-6">
                        <form action="{{ route('student.laporan.export') }}" method="GET" class="flex flex-col sm:flex-row items-end gap-4 sm:gap-5">
                            <div class="flex-1 w-full text-left">
                                <label class="text-[9px] sm:text-[10px] font-black text-cyan-500 uppercase tracking-widest mb-2 block ml-1">Dari Tanggal</label>
                                <div class="relative">
                                    <input type="date" name="start_date" class="w-full px-4 sm:px-5 py-3.5 sm:py-4 bg-gray-50 border border-gray-200 rounded-xl font-bold text-xs sm:text-sm text-gray-700 outline-none focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-300 shadow-inner transition-all placeholder:text-gray-300">
                                </div>
                            </div>
                            <div class="flex-1 w-full text-left">
                                <label class="text-[9px] sm:text-[10px] font-black text-cyan-500 uppercase tracking-widest mb-2 block ml-1">Sampai Tanggal</label>
                                <div class="relative">
                                    <input type="date" name="end_date" class="w-full px-4 sm:px-5 py-3.5 sm:py-4 bg-gray-50 border border-gray-200 rounded-xl font-bold text-xs sm:text-sm text-gray-700 outline-none focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-300 shadow-inner transition-all placeholder:text-gray-300">
                                </div>
                            </div>
                            <button type="submit" class="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-3.5 bg-slate-900 text-white rounded-xl font-black text-[10px] sm:text-xs uppercase tracking-widest hover:bg-cyan-600 transition-all shadow-lg hover:shadow-cyan-500/30 flex items-center justify-center gap-2 active:scale-95">
                    ...
                    <div class="rounded-2xl sm:rounded-[2rem] bg-white shadow-sm border border-gray-100 overflow-hidden text-left flex flex-col h-full max-h-[550px]">
                        <div class="p-4 sm:p-6 border-b border-gray-50 flex items-center justify-between bg-gray-50/50">
                            <h3 class="text-[9px] sm:text-[10px] font-black text-gray-800 uppercase tracking-[0.2em]"><i class="bi bi-clock-history me-1.5 sm:me-2 text-cyan-600"></i> Log Aktivitas Sirkulasi</h3>
                        </div>
                            ...
                            <table class="w-full text-left text-xs lg:text-sm min-w-[650px]">
                                <thead class="sticky top-0 bg-white z-10">
                                    <tr class="text-[7px] sm:text-[8px] lg:text-[9px] uppercase tracking-[0.2em] text-gray-400 font-black border-b border-gray-50">
                                        <th class="px-4 sm:px-6 py-3 sm:py-5">Buku / Paket</th>
                                ...
                                <tbody class="divide-y divide-gray-50">
                                    ...
                                    @php
                                        ...
                                        $subName = $itemCount > 1 ? $itemCount . ' Judul Buku' : 'ISBN: ' . $first->item->asset_code;
                                        $iconBox = $itemCount > 1 ? 'bi-journals' : 'bi-book-half';
                                    ...
                                    @endphp
                                    <tr class="group hover:bg-cyan-50/30 transition-all duration-200"
                                        x-show="searchQuery === '' || '{{ $searchString }}'.includes(searchQuery.toLowerCase())">
                                        
                                        <td class="px-4 sm:px-6 py-3 sm:py-4 text-left">
                                            <div class="flex items-center gap-2 sm:gap-3">
                                                <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-cyan-50 text-cyan-600 flex items-center justify-center font-black text-xs sm:text-sm shadow-inner uppercase border border-cyan-100 flex-shrink-0">
                                                    <i class="{{ $iconBox }}"></i>
                                                </div>
                                        ...
                                        <td class="px-4 sm:px-6 py-3 sm:py-4 text-left">
                                            <div class="flex flex-col">
                                                ...
                                                <div class="flex items-center gap-1 sm:gap-1.5 mt-1 sm:mt-1.5">
                                                    ...
                                                    <span class="text-[7px] sm:text-[8px] lg:text-[9px] font-bold {{ $isOverdue ? 'text-red-600 animate-pulse' : 'text-cyan-600' }} uppercase whitespace-nowrap">
                                        ...
                                        <td class="px-4 sm:px-6 py-3 sm:py-4 text-center">
                                            <button type="button" @click='showLoanDetail({{ $group->map(fn($l) => ["buku" => $l->item->name, "jumlah" => $l->quantity, "kode" => $l->item->asset_code, "kondisi" => $l->return_condition, "denda" => $l->fine_amount, "hilang" => $l->lost_quantity, "note_rusak" => $l->return_note])->toJson() }}, @json($titleName), "{{ $totalQty }}", "{{ $first->created_at->format("d M Y, H:i") }}", "{{ $statusText }}", "{{ $first->duration_amount }} {{ $first->duration_unit }}", "{{ $deadline->format("d M Y, H:i") }}", @json($first->reason), @json($first->admin_note ?? "Tidak ada catatan dari Petugas."), "{{ $first->return_date ? \Carbon\Carbon::parse($first->return_date)->format("d M Y, H:i") : "-" }}", "{{ $first->return_condition ?? "-" }}", "{{ $first->rating ?? 0 }}", "{{ number_format($group->sum("fine_amount"), 0, ",", ".") }}")' 
                                            class="w-7 h-7 sm:w-9 sm:h-9 rounded-lg sm:rounded-xl bg-gray-50 text-gray-400 hover:bg-cyan-600 hover:text-white transition-all flex items-center justify-center shadow-sm mx-auto border border-gray-100">
                                    ...
                                    <tr>
                                        <td colspan="5" class="py-16 sm:py-20 text-center">
                                            ...
                                            <p class="text-gray-400 font-bold uppercase text-[8px] sm:text-[10px] tracking-widest italic">Belum ada riwayat sirkulasi.</p>
                                        </td>
                                    </tr>
                                    @endforelse
                                </tbody>
                ...
                <div x-show="activeTab === 'laporan'" x-transition x-cloak class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 text-left">
                    ...
                    <div x-show="searchQuery === '' || '{{ strtolower($report->item->name) }}'.includes(searchQuery.toLowerCase())" 
                         class="p-5 sm:p-6 rounded-2xl sm:rounded-[2rem] border border-gray-100 bg-white hover:border-cyan-200 hover:shadow-xl transition-all relative overflow-hidden group flex flex-col justify-between min-h-[220px] sm:min-h-[250px] h-full text-left shadow-sm">
                        
                        <div class="absolute top-0 left-0 w-1 sm:w-1.5 h-full 
                            {{ $report->status === 'pending' ? 'bg-orange-500' : '' }}
                            {{ $report->status === 'process' ? 'bg-cyan-500' : '' }}
                            {{ $report->status === 'completed' || $report->status === 'fixed' ? 'bg-emerald-500' : '' }}">
                        </div>

                        <div class="absolute top-4 right-4 sm:top-5 sm:right-5 z-20">
                            <button type="button" @click='showReportDetail(@json($report->item->name), @json($report->item->asset_code), "{{ $report->created_at->format("d M Y, H:i") }}", @json($report->description), "{{ $report->status }}", "{{ $report->severity ?? "Ringan" }}", @json($report->admin_note ?? "Belum ada tanggapan petugas."), @json($report->photo_path ? asset("storage/".$report->photo_path) : null))' 
                            class="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white border border-gray-100 text-gray-400 hover:text-cyan-600 hover:border-cyan-100 flex items-center justify-center shadow-sm transition-all cursor-pointer">
                        ...
                        <div class="flex-1 pr-8 sm:pr-10 mb-3 sm:mb-4 relative z-10 pl-2 sm:pl-3">
                            <div class="mb-3">
                                ...
                                <p class="text-[8px] sm:text-[9px] font-black text-cyan-500 mt-1 sm:mt-1.5 uppercase text-left">ISBN: {{ $report->item->asset_code }}</p>
                            </div>
                            ...
                            <div class="mb-3 flex flex-wrap gap-1.5">
                                <span class="px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-md text-[7px] sm:text-[8px] font-black uppercase border shadow-sm leading-none
                                    ...
                                    {{ $report->status === 'process' ? 'bg-cyan-50 text-cyan-600 border-cyan-100' : '' }}
                            ...
                            @if($report->admin_note)
                                <div class="flex items-center gap-2 sm:gap-3">
                                    ...
                                    <div class="overflow-hidden">
                                        <p class="text-[7px] sm:text-[8px] font-bold text-slate-400 uppercase leading-none mb-0.5 sm:mb-1 truncate">Balasan Petugas</p>
                ...
                <div x-show="activeTab === 'rapot'" x-transition x-cloak>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6 mb-6 lg:mb-8 text-left">
                        <div class="p-5 lg:p-6 rounded-2xl lg:rounded-[2rem] glass-card border border-cyan-100 shadow-xl shadow-cyan-50 flex flex-col justify-between bg-white">
                            <div>
                                <span class="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-cyan-600 text-white text-[8px] sm:text-[9px] font-black rounded-lg uppercase tracking-widest mb-3 sm:mb-4 inline-block">Personal Rating</span>
                        ...
                        <div class="p-5 lg:p-6 rounded-2xl lg:rounded-[2rem] bg-white border border-gray-100 shadow-sm flex flex-col justify-between">
                            <div>
                                <span class="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-slate-900 text-white text-[8px] sm:text-[9px] font-black rounded-lg uppercase tracking-widest mb-3 sm:mb-4 inline-block">Member Average</span>
                                <p class="text-[9px] sm:text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Rata-rata Member</p>
                                <h3 class="text-3xl sm:text-4xl font-black text-gray-900">{{ number_format($classAverage ?? 0, 1) }}</h3>
                            </div>
                            <p class="text-[8px] sm:text-[10px] font-bold text-gray-500 italic mt-3 sm:mt-4 uppercase tracking-tighter">Performa kolektif seluruh member</p>
                        </div>
                        ...
                        <div class="p-5 lg:p-6 rounded-2xl lg:rounded-[2rem] {{ $isBetter ? 'bg-emerald-50 border-emerald-100' : 'bg-orange-50 border-orange-100' }} border flex flex-col justify-center items-center text-center">
                            ...
                            <p class="text-[8px] sm:text-[10px] font-bold opacity-70 mt-1 uppercase">Selisih {{ number_format($diff, 1) }} dari rata-rata global</p>
                        </div>
                    </div>

                    <div class="bg-white p-5 sm:p-6 lg:p-8 rounded-2xl lg:rounded-[2.5rem] border border-gray-100 shadow-sm text-left">
                        <div class="mb-4 sm:mb-6">
                            <h3 class="text-base sm:text-lg font-black text-gray-900 uppercase tracking-tight leading-none">Tren Kedisiplinan Anda</h3>
    ...
    <div x-show="modalTambah" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div x-show="modalTambah" x-transition.scale.95 class="relative w-full max-w-lg bg-white rounded-3xl sm:rounded-[2rem] shadow-2xl p-6 sm:p-8 lg:p-10 border border-white text-left overflow-y-auto max-h-[90vh] custom-scroll mt-auto sm:mt-0">
            <div class="flex justify-between items-start mb-5 sm:mb-6">
                <h3 class="text-xl sm:text-2xl font-black text-gray-900 font-jakarta uppercase tracking-tight leading-none">Ajukan Laporan Masalah</h3>
            ...
            {{-- ✅ FIX: ROUTE ACTION DIPERBAIKI SESUAI AKUN SISWA INDIVIDU (STUDENT) --}}
            <form action="{{ route('student.laporan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4 sm:space-y-6">
                @csrf
                
                <div class="text-left leading-none">
                    <label class="block text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 sm:mb-3">Pilih Buku Bermasalah</label>
                    <select name="loan_id" required class="w-full px-4 sm:px-5 py-3 sm:py-4 bg-gray-50 border border-gray-100 rounded-xl outline-none font-bold text-[11px] sm:text-xs text-gray-700 focus:ring-2 focus:ring-red-500/20 transition-all appearance-none shadow-sm cursor-pointer truncate">
                        <option value="">-- Pilih dari Riwayat Peminjaman Anda --</option>
                        @foreach($loans as $loan)
                            <option value="{{ $loan->id }}">{{ $loan->item->name }} (Dipinjam: {{ $loan->created_at->format('d M') }})</option>
                        @endforeach
                    </select>
                    <p class="text-[8px] sm:text-[9px] text-gray-400 mt-1.5 sm:mt-2 ml-1 italic">*Hanya buku yang pernah/sedang dipinjam yang bisa dilaporkan.</p>
                </div>
                ...
                <div class="text-left leading-none">
                    ...
                    <select name="severity" required class="w-full px-4 sm:px-5 py-3 sm:py-4 bg-gray-50 border border-gray-100 rounded-xl outline-none font-bold text-[11px] sm:text-xs text-gray-700 focus:ring-2 focus:ring-red-500/20 transition-all appearance-none shadow-sm cursor-pointer">
                        ...
                        <option value="Ringan">Ringan (Masih bisa dibaca dengan kendala kecil)</option>
                        <option value="Sedang">Sedang (Perlu perbaikan, ada kerusakan fisik)</option>
                        <option value="Parah">Rusak Parah / Hilang (Tidak bisa dikembalikan utuh)</option>
                    </select>
                </div>

                <div class="text-left leading-none">
                    <label class="block text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 sm:mb-3">Detail Laporan</label>
                    <textarea name="description" required rows="4" placeholder="Jelaskan detail kerusakan buku atau kronologi kehilangan..." class="w-full px-4 sm:px-5 py-3 sm:py-4 bg-gray-50 border border-gray-100 rounded-xl outline-none font-medium text-[11px] sm:text-xs focus:ring-2 focus:ring-red-500/20 transition-all shadow-sm resize-none"></textarea>
                </div>
                ...
                <div class="flex flex-col-reverse sm:flex-row gap-3 sm:gap-4 pt-2 sm:pt-4">
                    ...
                    <button type="submit" class="w-full sm:flex-1 px-6 py-3.5 sm:py-4 rounded-xl bg-red-600 text-white font-black text-[9px] sm:text-[10px] uppercase tracking-widest shadow-md hover:bg-red-700 active:scale-95 transition-all">Kirim Laporan</button>
                </div>
            </form>
        </div>
    </div>

    {{-- ✅ MODAL DETAIL PINTAR DENGAN HEADER STATIS --}}
    <div x-show="modalDetail" x-cloak class="fixed inset-0 z-[120] flex items-center justify-center p-4">
        ...
        <div x-show="modalDetail" x-transition.scale.95 class="relative w-full max-w-lg bg-white rounded-3xl sm:rounded-[2rem] shadow-2xl p-6 sm:p-8 lg:p-10 border border-white flex flex-col max-h-[90vh] text-left leading-none overflow-y-auto custom-scroll mt-auto sm:mt-0">
            ...
            <div class="flex justify-between items-start mb-5 sm:mb-6 text-left gap-3 border-b border-gray-100 pb-4">
                <div class="text-left overflow-hidden">
                    <h3 class="text-xl lg:text-2xl font-black text-gray-900 font-jakarta uppercase tracking-tight leading-tight truncate"
                        x-text="detailData.type === 'loan' ? 'DETAIL SIRKULASI' : 'DETAIL LAPORAN MASALAH'"></h3>
                </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-3 sm:gap-4 text-left">
                            <div class="p-4 sm:p-5 bg-slate-50 rounded-xl sm:rounded-2xl border border-slate-100">
                                ...
                                <p class="text-xs sm:text-sm font-black text-gray-800 truncate" x-text="detailData.qty + ' Buku'"></p>
                            </div>
                            <div class="p-4 sm:p-5 bg-slate-50 rounded-xl sm:rounded-2xl border border-slate-100">
                                <p class="text-[8px] sm:text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1 sm:mb-1.5">Status</p>
                                <p class="text-[10px] sm:text-xs font-black text-cyan-600 uppercase truncate" x-text="detailData.status"></p>
                            </div>
                        </div>
                        ...
                        <template x-if="detailData.is_paket || !detailData.is_paket"> 
                            <div class="p-4 sm:p-5 bg-white rounded-xl sm:rounded-2xl border border-slate-200 shadow-sm">
                                <p class="text-[8px] sm:text-[9px] font-black text-cyan-600 uppercase tracking-widest mb-2 sm:mb-3 border-b border-slate-100 pb-1.5 sm:pb-2"><i class="bi bi-book-half me-1"></i> Rincian Buku</p>
                                <div class="space-y-2.5 max-h-[200px] sm:max-h-[250px] overflow-y-auto custom-scroll pr-1">
                                    <template x-for="item in detailData.items" :key="item.kode">
                                        <div class="flex flex-col bg-slate-50 border border-slate-100 rounded-lg sm:rounded-xl overflow-hidden">
                                            ...
                                            <div class="flex justify-between items-start p-3 sm:p-4 bg-white">
                                                <div class="text-left leading-tight overflow-hidden pr-2">
                                                    ...
                                                    <p class="text-[8px] sm:text-[9px] font-bold text-gray-400 uppercase tracking-widest truncate" x-text="'ISBN: ' + item.kode"></p>
                                                </div>
                                                <span class="px-2 sm:px-2.5 py-0.5 sm:py-1 bg-white border border-cyan-200 rounded-md text-[8px] sm:text-[10px] font-black text-cyan-600 flex-shrink-0 shadow-sm whitespace-nowrap" x-text="item.jumlah + ' Buku'"></span>
                                            </div>
                        ...
                        <div class="p-4 sm:p-5 bg-white rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm space-y-3 sm:space-y-4">
                            <div class="flex items-center gap-1.5 sm:gap-2 text-cyan-600 font-black text-[9px] sm:text-[10px] uppercase tracking-widest mb-1">
                            ...
                            <div class="text-center pt-1 sm:pt-2">
                                <span class="px-2 sm:px-3 py-1 bg-cyan-50 text-cyan-600 rounded-md text-[7px] sm:text-[8px] font-black uppercase tracking-widest border border-cyan-100" x-text="'Durasi: ' + detailData.durasi"></span>
                            </div>
                        </div>
                        ...
                        <div x-show="detailData.return_date && detailData.return_date !== '-'" class="p-4 sm:p-5 bg-emerald-50/50 rounded-xl sm:rounded-[1.5rem] border border-emerald-100 text-left space-y-2 sm:space-y-3">
                            ...
                            <div x-show="detailData.user_rating > 0" class="flex justify-between items-center pt-1.5 sm:pt-2 border-t border-emerald-100/50 mt-1">
                                <span class="text-[8px] sm:text-[9px] font-bold text-emerald-600 uppercase tracking-widest">Rating Petugas</span>
                        ...
                        <div class="p-4 sm:p-5 bg-slate-900 rounded-xl sm:rounded-2xl border border-slate-800 text-left shadow-xl">
                            <p class="text-[7px] sm:text-[8px] font-black text-cyan-400 uppercase tracking-widest mb-1 sm:mb-1.5">Catatan Petugas Perpustakaan:</p>
                ...
                <template x-if="detailData.type === 'report'">
                    <div class="space-y-4 sm:space-y-5">
                        ...
                        <div class="p-4 sm:p-5 bg-white rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm flex items-center gap-3">
                            <div class="w-10 h-10 rounded-xl bg-red-50 text-red-600 flex items-center justify-center text-base sm:text-lg flex-shrink-0"><i class="bi bi-book"></i></div>
                            <div class="overflow-hidden text-left leading-tight">
                                ...
                                <p class="text-[8px] sm:text-[9px] font-bold text-gray-400 uppercase tracking-widest truncate" x-text="'ISBN: ' + detailData.asset_code"></p>
                            </div>
                        </div>
                        ...
                        <template x-if="detailData.photo">
                            <div class="w-full h-40 sm:h-48 bg-gray-100 rounded-xl sm:rounded-[1.5rem] overflow-hidden border border-gray-200 relative group shadow-inner">
                                ...
                                <div class="absolute bottom-2 sm:bottom-3 left-2 sm:left-3 bg-black/60 text-white px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-md backdrop-blur-md border border-white/20">
                                    <p class="text-[7px] sm:text-[8px] font-bold uppercase tracking-widest flex items-center gap-1.5"><i class="bi bi-image-fill"></i> Bukti Kerusakan</p>
                                </div>
                            </div>
                        </template>
                        ...
                        <div class="grid grid-cols-2 gap-3 sm:gap-4">
                            ...
                            <div class="p-3 sm:p-4 bg-slate-50 rounded-xl sm:rounded-2xl border border-slate-100">
                                ...
                                <p class="text-[9px] sm:text-[10px] font-black uppercase truncate" 
                                   :class="detailData.status === 'pending' ? 'text-orange-500' : (detailData.status === 'process' ? 'text-cyan-600' : 'text-emerald-600')"
                        ...
                        <div class="p-4 sm:p-5 bg-slate-900 rounded-xl sm:rounded-2xl border border-slate-800 shadow-xl relative overflow-hidden text-left">
                            <div class="relative z-10">
                                <p class="text-[7px] sm:text-[8px] font-black text-cyan-400 uppercase tracking-widest mb-1.5 sm:mb-2">Tanggapan Petugas:</p>
            ...
            <button @click="modalDetail = false" class="w-full mt-6 py-3.5 sm:py-4 bg-slate-900 text-white rounded-xl sm:rounded-2xl font-black text-[9px] sm:text-[10px] uppercase tracking-[0.2em] shadow-lg hover:bg-cyan-600 active:scale-95 transition-all border border-slate-800 hover:border-cyan-500">Tutup Panel</button>
        </div>
    </div>
    ...
    <script>
        function studentReportPage() {
            return {
                ...
                showLoanDetail(items, title, qty, date, status, durasi, tenggat, desc, feedback, return_date, condition, user_rating, total_denda) {
                    this.detailData = {
                        type: 'loan', is_paket: items.length > 1, items, item: title, qty, date, status, durasi, tenggat, desc, feedback, return_date, condition, user_rating, total_denda
                    };
                    this.modalDetail = true;
                },

                showReportDetail(item, asset_code, date, desc, status, severity, feedback, photo) {
                    this.detailData = {
                        type: 'report', item, asset_code, date, desc, status, severity, feedback, photo
                    };
                    this.modalDetail = true;
                },
                ...
                initChart() {
                    ...
                    var options = {
                        series: [{ name: 'Rating Kelas', data: chartData }],
                        chart: { type: 'area', height: 350, toolbar: { show: false }, zoom: { enabled: false } },
                        dataLabels: { enabled: false },
                        stroke: { curve: 'smooth', width: 4, colors: ['#0891b2'] },
                        xaxis: { categories: chartLabels, labels: { style: { colors: '#94a3b8', fontWeight: 800 } } },
                        yaxis: { max: 5, min: 0, labels: { style: { colors: '#94a3b8', fontWeight: 800 } } },
                        fill: { type: 'gradient', gradient: { shadeIntensity: 1, opacityFrom: 0.45, opacityTo: 0.05 } },
                        grid: { borderColor: '#f1f5f9' },
                        markers: { size: 6, colors: ['#0891b2'], strokeColors: '#fff', strokeWidth: 3 }
                    };
