...
<div x-data="{ 
    ...
    <header class="header-floating pointer-events-auto mx-auto max-w-[1550px] rounded-[2.5rem] shadow-[0_20px_50px_-15px_rgba(0,0,0,0.05)] border border-white/50 flex justify-between items-center px-10 py-4 transition-all duration-300">
        ...
        <div class="flex items-center gap-4 flex-shrink-0">
            ...
            <div class="leading-tight hidden sm:block text-left">
                ...
                <p class="text-[9px] font-bold text-blue-500 uppercase tracking-[0.2em] mt-1.5">Student Access</p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-4 flex-shrink-0">
            ...
            {{-- Notifikasi (DINAMIS) --}}
            <div class="relative">
                <button @click="notifOpen = !notifOpen" class="w-12 h-12 flex items-center justify-center rounded-2xl bg-white text-gray-400 border border-gray-100 hover:text-blue-600 hover:border-blue-100 transition-all shadow-sm relative">
                ...
                <div x-show="notifOpen" @click.away="notifOpen = false" x-cloak x-transition.origin.top.right 
                                ...
                                <div class="flex gap-3 text-left">
                                    ...
                                    @if(!$notification->read_at)
                                        <div class="flex-shrink-0 mt-2">
                                            <span class="block w-1.5 h-1.5 bg-blue-500 rounded-full"></span>
                                        </div>
                                    @endif
                                </div>
                            </a>
                    ...
                    @if(Auth::user()->notifications->count() > 0)
                        <div class="p-3 text-center border-t border-gray-50 bg-gray-50/30">
                            <button class="text-[10px] font-bold text-gray-500 hover:text-blue-600 uppercase tracking-widest">Lihat Semua</button>
                        </div>
                    @endif
                </div>
            </div>
            ...
            {{-- PROFIL BUTTON --}}
            <div class="relative">
                <button @click="profileOpen = !profileOpen" class="flex items-center gap-3 pl-2 pr-5 py-2 rounded-[1.8rem] bg-white border border-gray-100 shadow-sm hover:border-blue-100 transition-all group">
                    {{-- ✅ FOTO PROFIL DINAMIS --}}
                    <img src="{{ Auth::user()->profile_photo_url }}" class="w-9 h-9 rounded-xl object-cover shadow-sm border border-blue-50" alt="{{ Auth::user()->name }}">
                    ...
                    <div class="hidden sm:block text-left leading-none">
                        ...
                        <p class="text-[8px] font-bold text-blue-500 tracking-widest uppercase mt-1">Akun Kelas</p>
                    </div>
                    <i class="bi bi-chevron-down text-[10px] text-gray-400 ml-1 group-hover:text-blue-600"></i>
                </button>
                ...
                <div x-show="profileOpen" @click.away="profileOpen = false" x-cloak x-transition.origin.top.right 
                    ...
                    <div class="p-8 text-center border-b border-gray-50 bg-blue-50/20">
                    ...
                    <div class="p-3 space-y-1">
                        ...
                        <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-5 py-4 text-xs font-bold text-gray-600 rounded-2xl hover:bg-blue-50 hover:text-blue-600 transition-all">
                            <i class="bi bi-person-badge text-base"></i> Edit Profile Kelas
                        </a>
    ...
    <div x-show="modalLogout" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center p-4 pointer-events-auto">
        ...
        <div x-show="modalLogout" x-transition.scale.95 class="relative w-full max-w-sm bg-white rounded-[3rem] shadow-2xl p-10 text-center border border-white">
            ...
            <h3 class="text-2xl font-black text-gray-900 mb-2 font-jakarta">Logout Sesi?</h3>
            <p class="text-sm text-gray-500 mb-10 leading-relaxed font-medium">
                Sesi kelas akan diakhiri. Pastikan barang sudah dikembalikan dan laporan terisi.
            </p>
            ...
            <div class="flex gap-4">
                ...
                <form action="{{ route('logout') }}" method="POST" class="flex-1">
                    @csrf
                    <button type="submit" class="w-full px-6 py-4 rounded-2xl bg-[#EF4444] text-white font-bold text-xs uppercase tracking-widest shadow-xl shadow-red-200 hover:bg-red-600 active:scale-95 transition-all">
                        Ya, Keluar
                    </button>
                </form>
            </div>
