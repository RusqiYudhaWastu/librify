...
<head>
    ...
    <title>Pinjam Buku - Librify Siswa</title>
    ...
    <style>
        ...
        .custom-scroll::-webkit-scrollbar-thumb { background: #0ea5e9; border-radius: 20px; }
    </style>
</head>
...
<body class="antialiased flex h-screen w-full overflow-hidden text-left font-jakarta" 
                ...
                <div class="flex flex-col xl:flex-row xl:items-end justify-between gap-6">
                    <div class="text-left w-full xl:w-auto">
                        <div class="flex items-center gap-3 mb-4">
                            <h2 class="text-3xl font-black text-gray-900 tracking-tight uppercase leading-none">Sirkulasi & Peminjaman</h2>
                        ...
                        <div class="flex bg-gray-200/50 p-1.5 rounded-2xl gap-1 w-fit border border-gray-100">
                            <button @click="activeTab = 'booking'; searchQuery = ''" :class="activeTab === 'booking' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-blue-500'" class="px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Peminjaman Buku</button>
                ...
                <div x-show="activeTab === 'booking'" x-transition>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        ...
                        <div class="bg-slate-900 p-6 rounded-[2.5rem] shadow-xl flex items-center gap-5 text-white text-left border border-slate-800 relative overflow-hidden">
                            ...
                            <div class="text-left leading-none relative z-10">
                                <p class="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Member</p>
                                <p class="text-lg font-black uppercase leading-none tracking-tight">{{ Auth::user()->name }}</p>
                            </div>
                        </div>
                    </div>

                    {{-- REQUEST HISTORY TABLE (GROUPED) --}}
                    <div class="rounded-[2.5rem] bg-white shadow-sm border border-gray-100 overflow-hidden text-left">
                        <div class="p-8 border-b border-gray-50 flex items-center justify-between bg-gray-50/30">
                            <h3 class="text-[10px] font-black text-gray-800 uppercase tracking-[0.2em]">Monitoring Sirkulasi Berjalan</h3>
                        </div>
                            ...
                            <table class="w-full text-left text-sm">
                                <thead>
                                    <tr class="text-[10px] uppercase tracking-[0.2em] text-gray-400 font-black bg-white">
                                        ...
                                        <th class="px-8 py-6">Jumlah Buku</th>
    ...
    <div x-show="modalBooking" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div x-show="modalBooking" x-transition.scale.95 class="relative w-full max-w-3xl bg-white rounded-[3rem] shadow-2xl p-10 border border-white text-left overflow-y-auto max-h-[90vh] custom-scroll">
            ...
            <form action="{{ route('student.request.store') }}" method="POST" class="space-y-6 text-left">
                @csrf
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 leading-none">
                    ...
                    <div class="text-left leading-none flex flex-col">
                        ...
                        <textarea name="reason" required placeholder="Jelaskan untuk keperluan apa Anda meminjam buku ini..." class="w-full flex-1 px-6 py-5 bg-gray-50 border border-gray-100 rounded-2xl outline-none font-medium text-xs focus:ring-4 focus:ring-blue-500/10 transition-all shadow-inner resize-none"></textarea>
                    </div>
                </div>
    ...
    {{-- Script Alpine --}}
    <script>
        function studentIndividuRequest() {
            return {
                sidebarOpen: false,
                activeTab: 'booking',
                searchQuery: '',
