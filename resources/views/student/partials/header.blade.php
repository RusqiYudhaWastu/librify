<style>
    ...
    .notif-scroll::-webkit-scrollbar-thumb { background-color: #a5f3fc; border-radius: 10px; }
</style>
...
<div x-data="{ 
    ...
    <header class="header-floating pointer-events-auto mx-auto max-w-[1550px] rounded-2xl md:rounded-[2.5rem] shadow-[0_10px_40px_-15px_rgba(0,0,0,0.08)] border border-white/50 flex justify-between items-center px-4 md:px-10 py-3 md:py-4 transition-all duration-300">
        ...
        {{-- 1. BRAND SECTION (Librify Update) --}}
        <div class="flex items-center gap-3 md:gap-4 flex-shrink-0">
            
            {{-- TOMBOL HAMBURGER (Mobile Only) --}}
            <button type="button" @click="sidebarOpen = true" class="md:hidden -ml-2 mr-1 p-2 text-gray-500 hover:text-cyan-600 focus:outline-none transition-colors rounded-lg active:bg-gray-100">
                <i class="bi bi-list text-2xl leading-none"></i>
            </button>

            <div class="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-cyan-500 text-white flex items-center justify-center shadow-lg shadow-cyan-200">
            ...
            <div class="leading-tight text-left">
                ...
                <p class="text-[7px] md:text-[9px] font-bold text-cyan-500 uppercase tracking-[0.2em] mt-1 md:mt-1.5">Student Access</p>
            </div>
        </div>

        {{-- 2. DATE CENTER --}}
        <div class="hidden sm:flex flex-col items-center flex-1 px-4 text-center leading-none">
            <p class="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-0.5">Sirkulasi Perpustakaan</p>
        ...
        <div class="flex items-center gap-2 md:gap-4 flex-shrink-0">
            ...
            {{-- Notifikasi --}}
            <div class="relative">
                <button @click="notifOpen = !notifOpen" class="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-xl md:rounded-2xl bg-white text-gray-400 border border-gray-100 hover:text-cyan-600 hover:border-cyan-100 transition-all shadow-sm relative">
                ...
                <div x-show="notifOpen" @click.away="notifOpen = false" x-cloak x-transition.origin.top.right 
                    ...
                    <div class="p-5 border-b border-gray-50 bg-gray-50/50 flex justify-between items-center">
                        ...
                        @else
                            <span class="bg-cyan-100 text-cyan-600 text-[9px] font-black px-2 py-1 rounded-lg">SYSTEM</span>
                        @endif
                    </div>
                                    ...
                                    <div class="flex-shrink-0 mt-1">
                                        @php 
                                            ...
                                            $icon = [
                                                ...
                                                'info' => 'bi-info-circle-fill text-cyan-500'
                                            ][$type] ?? 'bi-info-circle-fill text-cyan-500';
                                        @endphp
                                        <i class="bi {{ $icon }}"></i>
                                    </div>
                                    <div class="flex-1 leading-tight text-left">
                                        <p class="text-[11px] font-bold text-gray-800 leading-tight mb-1">{{ $notification->data['title'] ?? 'Info Perpustakaan' }}</p>
            ...
            {{-- Profil Button --}}
            <div class="relative">
                <button @click="profileOpen = !profileOpen" class="flex items-center gap-2 md:gap-3 pl-1.5 md:pl-2 pr-1.5 md:pr-5 py-1.5 md:py-2 rounded-xl md:rounded-[1.8rem] bg-white border border-gray-100 shadow-sm hover:border-cyan-100 transition-all group">
                    <img src="{{ Auth::user()->profile_photo_url }}" class="w-7 h-7 md:w-9 md:h-9 rounded-lg md:rounded-xl object-cover shadow-sm border border-cyan-50" alt="{{ Auth::user()->name }}">
                    ...
                    <div class="hidden sm:block text-left leading-none">
                        ...
                        <p class="text-[7px] md:text-[8px] font-bold text-cyan-500 tracking-widest uppercase mt-1">Reader Member</p>
                    </div>
                    <i class="bi bi-chevron-down text-[8px] md:text-[10px] text-gray-400 md:ml-1 group-hover:text-cyan-600"></i>
                </button>
                ...
                <div x-show="profileOpen" @click.away="profileOpen = false" x-cloak x-transition.origin.top.right 
                    ...
                    <div class="p-6 text-center border-b border-gray-50 bg-cyan-50/20">
                    ...
                    <div class="p-2 space-y-1">
                        <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-4 py-3 text-[11px] md:text-xs font-bold text-gray-600 rounded-xl hover:bg-cyan-50 hover:text-cyan-600 transition-all">
                            <i class="bi bi-person-badge text-base"></i> Profil Saya
                        </a>
    ...
    <div x-show="modalLogout" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center p-4 pointer-events-auto text-left">
        ...
        <div x-show="modalLogout" x-transition.scale.95 class="relative w-full max-w-[320px] md:max-w-sm bg-white rounded-[2rem] md:rounded-[3rem] shadow-2xl p-8 md:p-10 text-center border border-white">
            ...
            <p class="text-[12px] md:text-sm text-gray-500 mb-8 leading-relaxed font-medium">Apakah Anda yakin ingin keluar dari akun Librify?</p>
