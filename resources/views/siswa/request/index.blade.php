...
<head>
    ...
    <title>Booking Kelas - TekniLog Siswa</title>
    ...
    <style>
        ...
        .custom-scroll::-webkit-scrollbar-thumb { background: #3b82f6; border-radius: 20px; } /* Blue Scrollbar */
    </style>
</head>
    ...
    <div class="flex flex-1 flex-col h-full min-w-0 overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto p-6 lg:p-10 pt-2 custom-scroll">
            <div class="mx-auto w-full max-w-[1550px] space-y-8">
                ...
                {{-- Alert System --}}
                @if(session('success'))
                <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)" class="bg-blue-600 text-white px-6 py-4 rounded-[2rem] shadow-lg flex justify-between items-center transition-all relative z-50">
                ...
                <div class="flex flex-col xl:flex-row xl:items-end justify-between gap-6">
                    ...
                    <div class="text-left w-full xl:w-auto">
                        <div class="flex items-center gap-3 mb-4">
                            <h2 class="text-3xl font-black text-gray-900 tracking-tight uppercase leading-none">Manajemen Alat Kelas</h2>
                            <span class="px-3 py-1.5 bg-blue-600 text-white text-[10px] font-black rounded-lg uppercase tracking-widest shadow-lg shadow-blue-100">
                                {{ Auth::user()->department ? Auth::user()->department->name : 'Umum' }}
                            </span>
                        </div>
                        
                        {{-- Tab Menu --}}
                        <div class="flex bg-gray-200/50 p-1.5 rounded-2xl gap-1 w-fit border border-gray-100">
                            <button @click="activeTab = 'booking'; searchQuery = ''" :class="activeTab === 'booking' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-blue-500'" class="px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Peminjaman Kelas</button>
                    ...
                    <div class="flex flex-col md:flex-row items-center gap-4 w-full xl:w-auto">
                        ...
                        <div class="relative w-full md:w-80 group">
                            ...
                            <input type="text" x-model="searchQuery" placeholder="Cari nama alat, status, dll..." 
                                   class="w-full pl-12 pr-6 py-4 bg-white border border-gray-100 rounded-[1.5rem] outline-none font-bold text-xs shadow-sm focus:ring-4 focus:ring-blue-500/10 transition-all placeholder:text-gray-300">
                        </div>

                        {{-- Tombol Booking --}}
                        <template x-if="activeTab === 'booking'">
                            <button @click="modalBooking = true" class="w-full md:w-auto inline-flex items-center justify-center gap-3 rounded-[1.5rem] bg-blue-600 px-8 py-4 text-xs font-black text-white shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all active:scale-95 uppercase tracking-widest leading-none flex-shrink-0">
                ...
                <div x-show="activeTab === 'booking'" x-transition>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <div class="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm flex items-center gap-5">
                            ...
                            <div class="text-left leading-none">
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5">Total Transaksi</p>
                        ...
                        <div class="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm flex items-center gap-5 text-left">
                            ...
                            <div class="text-left leading-none">
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5">Sedang Diproses</p>
                        ...
                        <div class="bg-slate-900 p-6 rounded-[2.5rem] shadow-xl flex items-center gap-5 text-white text-left border border-slate-800 relative overflow-hidden">
                            ...
                            <div class="w-14 h-14 bg-white/10 text-blue-400 rounded-2xl flex items-center justify-center text-2xl shadow-inner"><i class="bi bi-people-fill"></i></div>
                    ...
                    <div class="rounded-[2.5rem] bg-white shadow-sm border border-gray-100 overflow-hidden text-left">
                        <div class="p-8 border-b border-gray-50 flex items-center justify-between bg-gray-50/30">
                            <h3 class="text-[10px] font-black text-gray-800 uppercase tracking-[0.2em]">Monitoring Inventaris Kelas</h3>
                        </div>
                                ...
                                <tbody class="divide-y divide-gray-50">
                                    ...
                                    <tr class="group hover:bg-gray-50/50 transition-all duration-200" 
                                        ...
                                        <td class="px-8 py-5 text-left">
                                            <div class="flex items-center gap-4">
                                                <div class="w-11 h-11 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center font-black text-xs shadow-inner uppercase">{{ substr($loan->item->name, 0, 2) }}</div>
                                        ...
                                        <td class="px-8 py-5 text-center">
                                            ...
                                            }; modalStatus = true" class="w-10 h-10 rounded-xl bg-gray-50 text-gray-400 hover:bg-blue-600 hover:text-white transition-all mx-auto flex items-center justify-center shadow-sm">
                                    ...
                                    <tr>
                                        <td colspan="5" class="py-20 text-center">
                                            ...
                                            <p class="text-gray-400 font-bold uppercase text-[10px] tracking-widest">Belum ada riwayat peminjaman kelas</p>
                                        </td>
                                    </tr>
                                    @endforelse
                                </tbody>
        ...
        <div x-show="modalBooking" x-transition.scale.95 class="relative w-full max-w-xl bg-white rounded-[3rem] shadow-2xl p-10 border border-white text-left overflow-y-auto max-h-[90vh] custom-scroll">
            <div class="mb-10 text-left leading-none">
                ...
                <div class="flex items-center gap-2">
                    <span class="text-[9px] font-black text-blue-500 uppercase tracking-[0.2em] bg-blue-50 px-2 py-1 rounded">Inventaris {{ Auth::user()->department ? Auth::user()->department->name : 'Umum' }}</span>
                </div>
            </div>
            
            <form action="{{ route('siswa.request.store') }}" method="POST" class="space-y-6">
                @csrf
                <div class="space-y-6">
                    <div class="text-left leading-none">
                        <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Pilih Barang Ready</label>
                        <select name="item_id" required class="w-full px-6 py-5 bg-gray-50 border border-gray-100 rounded-2xl outline-none font-bold text-sm text-gray-700 focus:ring-4 focus:ring-blue-500/10 transition-all appearance-none shadow-inner">
                    ...
                    <div class="text-left leading-none">
                        ...
                        <textarea name="reason" required rows="4" placeholder="Jelaskan untuk praktek apa dan di ruangan mana..." class="w-full px-6 py-5 bg-gray-50 border border-gray-100 rounded-2xl outline-none font-medium text-sm focus:ring-4 focus:ring-blue-500/10 transition-all shadow-inner"></textarea>
                    </div>
                </div>
                ...
                <div class="flex gap-4 pt-6">
                    ...
                    <button type="submit" class="flex-1 px-6 py-5 rounded-[2rem] bg-blue-600 text-white font-black text-xs uppercase tracking-widest shadow-xl shadow-blue-100 hover:bg-blue-700 active:scale-95 transition-all">Kirim Pesanan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-show="modalStatus" x-cloak class="fixed inset-0 z-[120] flex items-center justify-center p-4">
        ...
        <div x-show="modalStatus" x-transition.scale.95 class="relative w-full max-w-lg bg-white rounded-[3rem] shadow-2xl p-10 border border-white flex flex-col max-h-[90vh] text-left leading-none overflow-y-auto custom-scroll">
            ...
            <div class="mb-8 text-left leading-none">
                <span class="text-[9px] font-black text-blue-500 uppercase tracking-[0.2em] bg-blue-50 px-2 py-1 rounded mb-2 inline-block">Detail Peminjaman</span>
                    ...
                    <div class="flex flex-col">
                        ...
                        <span class="text-sm font-black uppercase" 
                              :class="{
                                  ...
                                  'text-blue-600': selectedRequest.status === 'approved',
            ...
            <button @click="modalStatus = false" class="w-full mt-8 py-5 bg-slate-900 text-white rounded-[2rem] font-black text-[10px] uppercase tracking-[0.2em] shadow-xl hover:bg-blue-600 active:scale-95 transition-all">Tutup Informasi</button>
        </div>
    </div>
